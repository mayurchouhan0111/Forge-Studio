...
const Testimonials: React.FC<TestimonialsProps> = ({ onOpenBooking }) => {
  ...
  return (
    <section id="testimonials" className="py-24 px-4 sm:px-6 bg-slate-50 relative overflow-hidden">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-indigo-200/40 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto z-10 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 mb-12 sm:mb-16">
          {reviews.map((rev, idx) => (
            <motion.div
              key={rev.id}
              className="glass-card glass-card-hover p-6 sm:p-8 rounded-3xl border border-slate-200 bg-white flex flex-col justify-between relative shadow-md hover:shadow-2xl hover:shadow-indigo-500/15 overflow-hidden group cursor-default"
            ...
            >
              ...
              <div className="space-y-4 pt-1">
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <img
                      src={rev.avatar}
                      alt={rev.name}
                      className="w-12 h-12 sm:w-13 sm:h-13 rounded-full object-cover border-2 border-indigo-600 shadow-md shadow-indigo-500/20 group-hover:scale-105 transition-transform"
                    />
                    <div>
                      <h4 className="font-black text-slate-900 text-sm sm:text-base group-hover:text-indigo-600 transition-colors">{rev.name}</h4>
                      <p className="text-xs text-slate-500 font-medium">{rev.role}</p>
                    </div>
                  </div>
                  <Quote className="w-6 h-6 sm:w-8 sm:h-8 text-indigo-300 flex-shrink-0 group-hover:rotate-12 transition-transform" />
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-2">
                  ...
                  <span className="px-2.5 py-0.5 bg-indigo-50 border border-indigo-200 rounded-full text-[10px] sm:text-[11px] font-mono text-indigo-700 font-bold">
                    {rev.project}
                  </span>
                </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center gap-2 text-xs font-semibold text-emerald-600 mt-6">
        ...
        <div className="glass-card p-6 sm:p-10 rounded-3xl border border-indigo-200 bg-gradient-to-r from-indigo-600 via-violet-600 to-indigo-700 text-white text-center sm:text-left max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-5 sm:gap-6 shadow-xl shadow-indigo-500/25">
          <div className="space-y-1">
            <h3 className="text-lg sm:text-xl font-bold text-white">Ready to join our roster of successful clients?</h3>
            <p className="text-xs text-indigo-100">Reserve your Q3 sprint slot before our calendar fills up.</p>
          </div>
          <button
            onClick={() => onOpenBooking('Reviews CTA')}
            className="w-full sm:w-auto px-6 py-3.5 bg-white hover:bg-slate-50 text-indigo-600 font-extrabold text-xs uppercase tracking-wider rounded-xl shadow-lg whitespace-nowrap active:scale-[0.98] transition-transform"
          >
            Book Discovery Call
          </button>
        </div>
